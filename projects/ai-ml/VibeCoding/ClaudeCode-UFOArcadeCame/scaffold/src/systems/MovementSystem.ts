...
/** Moves the player left/right, clamped to playfield bounds, no drift (F1 AC1-AC3). */
export function updateMovement(world: World, input: InputSnapshot, dt: number): void {
  const speed =
    world.effects.speedRemaining > 0 ? PLAYER_BASE_SPEED * SPEED_MULTIPLIER : PLAYER_BASE_SPEED;
